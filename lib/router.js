...
KeywordPageController = RouteController.extend({
  template: 'keywordPage',
    waitOn: function() {
    return [Meteor.subscribe('singleKeyword', this.params._id), Meteor.subscribe('moviesWithKeyword', this.params._id, this.findOptions()), Meteor.subscribe('years')]
  },
  findOptions: function() {
    // return {sort: {keyword_count: -1}}
    return {sort: this.sort}
...
Router.route('/actor/:_id', {
  ...
  template: 'actorPage',
  waitOn: function() {
    return [Meteor.subscribe('actorsMovies', this.params._id), Meteor.subscribe('actor', this.params._id)]
  },
